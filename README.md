...
7. **Prod vs dev**
   - In production (Vercel), vercel.json settings matter:
     - Correct routing to API
     - Correct environment variables configured in Vercel dashboard

---

## How to confidently extend the codebase

When adding new features:

1. **Start at the UX**:  
   - What new screen or action is needed? Add/modify a component in components and wire it into App.js.

2. **Define the API contract**:  
   - Decide the endpoint path, method, and JSON structure. Implement the route in index.js.

3. **Use DB ops layer**:  
   - Add/change functions in dboperation.js and schemas in schema.js if data model changes.

4. **Apply auth as needed**:  
   - Decide if the route is public or protected and use middleware from authenticate.js accordingly.

5. **Wire frontend to backend**:  
   - Use your frontend environment variable for the base URL.
   - Call the new endpoint from the appropriate component.

If you tell me what specific feature you want to add or what part of the stack you want to modify first, I can walk you through a concrete change step by step.
